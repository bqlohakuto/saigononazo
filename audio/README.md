...
効果音と劇中音楽は `game-audio.js` の Web Audio で再生します。再生ごとに `AudioBufferSourceNode` を作り、終了時に切断します。HTMLの音声プレーヤーやOSのメディア再生ボタンには再生処理を割り当てません。
...
音量設定は共通の GainNode に適用します。基準音量は耳鳴り0.5、扉0.65、記憶のピアノ0.7、回想の吹奏楽0.65。設定値を0にすると全音声が無音になります。
...
第一の部屋の正解音階「ソ・ラ・ファ・ミ・ド・レ・ド・シ」を演奏します。`memory_melody.mid` も同じ8音です。旧9音版に入っていた8番目の余分な「ミ」は、2026年9月20日のシナリオ更新でMIDIとWAVの両方から除去しました。WAVは音の境目に近い波形の一致点で旧「ミ」の区間だけを切り、ほかの音色と余韻を維持しています。

### 回想の吹奏楽 `memory_band.mp3`

- 原本：ユーザー提供の `Notes_for_a_New_Morning.mp3`
- 長さ：約66.586秒
- 形式：MPEG-1 Layer III、192 kbps、44.1 kHz、joint stereo
- SHA-256：`6267D2788FB9F19879A99D2E52E035D1A84742E390388622247A011732424241`
- メタデータ：通常の作者・著作権・ライセンスタグはありません。埋め込みC2PAには Google Generative AI による作成と SynthID 透かしが記録されています。

「どこからか、音楽が聴こえてきた。」の表示開始時に再生し、「何度か同じ場所を繰り返し、ようやく音が揃ってきたころ――。」の表示開始時にまだ再生中なら2秒かけてフェードアウトします。すでに自然終了している場合は何もしません。音声が利用できない環境でもゲームの進行は止まりません。
